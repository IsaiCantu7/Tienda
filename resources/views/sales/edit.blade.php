            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white dark:bg-gray-800">
                    <form method="POST" action="{{ route('sales.update', $sale->id) }}">
                        ...
                        <!-- ID del Producto -->
                        <div class="mb-4">
                            <label for="id_product" class="block font-medium text-sm text-gray-700 dark:text-gray-300">{{ ('ID del Producto') }}</label>
                            <select id="id_product" class="block mt-1 w-full" name="id_product" required autofocus>
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}" @if ($sale->id_product == $product->id) selected @endif>{{ $product->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <!-- ID de la Categoría -->
                        <div class="mb-4">
                            <label for="id_category" class="block font-medium text-sm text-gray-700 dark:text-gray-300">{{ ('ID de la Categoría') }}</label>
                            <select id="id_category" class="block mt-1 w-full" name="id_category" required>
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" @if ($sale->id_category == $category->id) selected @endif>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <!-- ID del Cliente -->
                        <div class="mb-4">
                            <label for="id_customer" class="block font-medium text-sm text-gray-700 dark:text-gray-300">{{ ('ID del Cliente') }}</label>
                            <select id="id_customer" class="block mt-1 w-full" name="id_customer" required>
                                @foreach ($customers as $customer)
                                    <option value="{{ $customer->id }}" @if ($sale->id_customer == $customer->id) selected @endif>{{ $customer->first_name }} {{ $customer->last_name }}</option>
                                @endforeach
                            </select>
                        </div>
